...
from flask_babel import _, get_locale  # 翻译函数，仅仅是一个下划线


@app.before_request
def before_request():  # 记录用户的最后访问时间
...
@app.route('/user/<username>')  # <...> 动态展示格式
@login_required
def user(username):
    user = User.query.filter_by(username=username).first_or_404()      # 工作方式与first()完全相同，但是在没有结果的情况下会自动发送404 error给客户端。
    page = request.args.get('page', 1, type=int)
...
def explore():  # 新增发现页
    ...
    prev_url = url_for('explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('index.html', title=_('探索'),
                           posts=posts.items, next_url=next_url,
                           prev_url=prev_url)
